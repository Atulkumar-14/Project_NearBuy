...
class UserVerification(Base):
    ...
    ver_id: Mapped[uuid.UUID] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
...
class OwnerSecurity(Base):
    ...
    sec_id: Mapped[uuid.UUID] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    owner_id: Mapped[int] = mapped_column(Integer, ForeignKey("Shop_Owners.owner_id", ondelete="CASCADE"), index=True)
    email_verified_at: Mapped[datetime | None] = mapped_column(DateTime)
    phone_verified_at: Mapped[datetime | None] = mapped_column(DateTime)
...
class AuthLog(Base):
    ...
    log_id: Mapped[uuid.UUID] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
